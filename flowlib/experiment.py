...
class Trainer(BaseTrainer):
    def __init__(
        self, y_conditional: bool = False, scheduler_params: Dict[str, Any] = None, **kwargs: Any
    ) -> None:
        super().__init__(**kwargs)

        self._y_conditional = y_conditional
        self._scheduler_params = scheduler_params or {}
        self._model: flowlib.FlowModel
        self._scheduler: flowlib.NoamScheduler

    def _set_model(self, model: nn.Module) -> None:

        assert isinstance(model, flowlib.FlowModel)
        self._model = model
        self._optimizer = optim.Adam(self._model.parameters())
        self._scheduler = flowlib.NoamScheduler(self._optimizer, **self._scheduler_params)

    def _train_step(self, data: Tensor, label: Tensor) -> Dict[str, Tensor]:

        label_or_null = label if self._y_conditional else None
        return self._model.loss_func(data, label_or_null)
    ...
    def _save_plots(self) -> None:
        ...
        with torch.no_grad():
            ...
            label = label[:16].to(self._device) if self._y_conditional else None
